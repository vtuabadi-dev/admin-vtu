...
function formatError(error: any) {
  ...
  if (error.message === "NOT_FOUND") {
    return NextResponse.json({ success: false, message: "Package type not found" }, { status: 404 });
  }
...
export async function GET(_request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const data = await masterDataService.getPackageTypeById(params.id);
    if (!data) return NextResponse.json({ success: false, message: "Package type not found" }, { status: 404 });
...
export async function PUT(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const parsed = updatePackageTypeSchema.safeParse(body);
    if (!parsed.success) {
      return NextResponse.json({ success: false, message: "Validation Error", data: parsed.error.format() }, { status: 400 });
    }

    const data = await masterDataService.updatePackageType(params.id, parsed.data);
    return NextResponse.json({ success: true, data });
  } catch (error) {
    return formatError(error);
  }
...
export async function DELETE(_request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    await masterDataService.deletePackageType(params.id);
