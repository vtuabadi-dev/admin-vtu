...
export async function GET(request: NextRequest) {
  try {
    ...
    const result = await masterDataService.getPackageTypes({ limit, offset, sort, order, search, isActive });
...
export async function POST(request: NextRequest) {
  try {
    ...
    const parsed = createPackageTypeSchema.safeParse(body);
    if (!parsed.success) {
      return NextResponse.json({ success: false, message: "Validation Error", data: parsed.error.format() }, { status: 400 });
    }

    const data = await masterDataService.createPackageType(parsed.data);
