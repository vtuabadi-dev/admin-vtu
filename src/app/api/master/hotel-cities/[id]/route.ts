...
const updateHotelCitySchema = z.object({
...
export async function GET(_request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const data = await masterDataService.getHotelCityById(params.id);
...
export async function PUT(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const parsed = updateHotelCitySchema.safeParse(body);
    if (!parsed.success) {
      return NextResponse.json({ success: false, message: "Validation Error", data: parsed.error.format() }, { status: 400 });
    }

    const data = await masterDataService.updateHotelCity(params.id, parsed.data);
    return NextResponse.json({ success: true, data });
  } catch (error) {
    return formatError(error);
  }
...
export async function DELETE(_request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    await masterDataService.deleteHotelCity(params.id);
