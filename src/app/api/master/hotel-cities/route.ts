...
const createHotelCitySchema = z.object({
...
export async function GET(request: NextRequest) {
  try {
    ...
    const result = await masterDataService.getHotelCities({ limit, offset, sort, order, search, isActive });
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Support "nama" and "kode" coming from UI inputs
    if (body.nama !== undefined && body.name === undefined) body.name = body.nama;
    if (body.kode !== undefined && body.code === undefined) body.code = body.kode;
    if (body.status === "Aktif") body.isActive = true;
    if (body.status === "Nonaktif") body.isActive = false;

    const parsed = createHotelCitySchema.safeParse(body);
    if (!parsed.success) {
      return NextResponse.json({ success: false, message: "Validation Error", data: parsed.error.format() }, { status: 400 });
    }

    const data = await masterDataService.createHotelCity(parsed.data);
