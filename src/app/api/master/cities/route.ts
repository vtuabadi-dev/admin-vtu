...
export async function GET(request: NextRequest) {
  try {
    ...
    const result = await masterDataService.getCities({ limit, offset, sort, order, search, isActive });
...
export async function POST(request: NextRequest) {
  try {
    ...
    // In CrudTab starting points only ask for "nama" and "kode". We default country to "Indonesia" or "Arab Saudi" based on name or code.
    // If country is missing, default it to "Indonesia".
    if (!body.country) {
      const nameLower = (body.nama || body.name || "").toLowerCase();
      if (nameLower.includes("mekkah") || nameLower.includes("makkah") || nameLower.includes("madinah") || nameLower.includes("jeddah")) {
        body.country = "Arab Saudi";
      } else {
    ...
    const data = await masterDataService.createCity(parsed.data);
