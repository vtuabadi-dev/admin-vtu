...
const WAKAF_BG_FOLDER_ID = process.env.GOOGLE_DRIVE_WAKAF_BG_FOLDER_ID || "1IDHH8jvRkLgl4mix7msF1XEB22f88MlA";
...
export async function GET() {
  ...
  try {
    ...
    // 1. Query for the latest image in Google Drive folder 1IDHH8jvRkLgl4mix7msF1XEB22f88MlA
    const query = `'${WAKAF_BG_FOLDER_ID}' in parents and mimeType contains 'image/' and trashed = false`;
    ...
    console.error("[Wakaf Quran Dynamic Background Error]:", error?.message || error);
    if (memoryCache) {
      return new NextResponse(memoryCache.buffer as any, {
        headers: { "Content-Type": memoryCache.mimeType },
      });
