...
export function generateInvoicePdf(data: InvoicePdfData): jsPDF {
  ...
  autoTable(doc, {
    startY: startY + 48,
    head: [
      ["NO", "DESKRIPSI PEMBAYARAN", "METODE", "TANGGAL", "NOMINAL"],
    ],
    body: tableRows,
    theme: "grid",
    headStyles: {
  ...
  doc.roundedRect(summaryBoxX, finalY, summaryBoxWidth, 34, 2, 2, "FD");

  doc.setFontSize(8.5);
  doc.setFont("helvetica", "normal");
  doc.setTextColor(100, 116, 139);

  doc.text("Total Tagihan Paket:", summaryBoxX + 4, finalY + 7);
  doc.setFont("helvetica", "bold");
  ...
  doc.setFont("helvetica", "normal");
  doc.setTextColor(100, 116, 139);
  doc.text("Total Terbayar Sebelumnya:", summaryBoxX + 4, finalY + 14);
  doc.setFont("helvetica", "bold");
  doc.setTextColor(30, 41, 59);
  doc.text(
  ...
  doc.rect(summaryBoxX + 2, finalY + 17.5, summaryBoxWidth - 4, 8, "F");

  doc.setFont("helvetica", "bold");
  doc.setTextColor(5, 150, 105);
  doc.text("Nominal Invoice Ini:", summaryBoxX + 4, finalY + 23);
  doc.text(
    `Rp ${data.nominal.toLocaleString("id-ID")}`,
    summaryBoxX + summaryBoxWidth - 4,
    finalY + 23,
  ...
  doc.setFont("helvetica", "normal");
  doc.setTextColor(100, 116, 139);
  doc.text("Sisa Tagihan Setelah Ini:", summaryBoxX + 4, finalY + 30);
  doc.setFont("helvetica", "bold");
  ...
  doc.text(
    `Rp ${(data.sisaTagihan !== undefined ? data.sisaTagihan : Math.max(0, (data.totalTagihan || 0) - (data.totalPembayaran || 0) - data.nominal)).toLocaleString("id-ID")}`,
    summaryBoxX + summaryBoxWidth - 4,
    finalY + 30,
    { align: "right" }
  );
  ...
  doc.text("CATATAN & KETENTUAN RESMI:", 14, finalY + 7);
  ...
  const terms = [
    ...
    "2. Pembayaran yang telah diverifikasi otomatis tercatat dalam sistem manifest tiket & akomodasi.",
    "3. Simpan dokumen ini sebagai bukti pelunasan dan konfirmasi keberangkatan ke Baitullah.",
    "4. Pertanyaan seputar pembayaran & jadwal hubungi Finance Desk VTU: 0812-3456-7890.",
  ];
  let termY = finalY + 13;
  ...
  doc.roundedRect(pageWidth - 64, signY + 10, 50, 18, 1.5, 1.5, "D");

  doc.setFont("helvetica", "bold");
  doc.setFontSize(7);
  doc.setTextColor(5, 150, 105);
  doc.text("VERIFIED & DIGITALLY SIGNED", pageWidth - 39, signY + 16, { align: "center" });
  doc.setFont("helvetica", "normal");
  doc.setFontSize(6.5);
  doc.setTextColor(71, 85, 105);
  doc.text(`AUTH: VTU-${data.invoiceNumber.slice(-8)}`, pageWidth - 39, signY + 21, { align: "center" });
  doc.text("FINANCE DEPARTMENT", pageWidth - 39, signY + 25, { align: "center" });
